...
export const getVerifiedContext = async (query) => {
	if (!isInitialized) {
		await initRAG();
	}
	if (!vectorStore) return null;

	// Debug trace for RAG queries
	console.log('🔍 RAG searching for:', query);

	// AGGRESSIVE: If query contains single node, ALWAYS return empty context
	if (query.includes('ONLY of the following components:') && query.includes('[') && query.includes(']')) {
		// Extract node count from query
		const nodeMatch = query.match(/\[([^\]]+)\]/);
		if (nodeMatch) {
			const nodes = nodeMatch[1].split(',').map(n => n.trim());
			if (nodes.length === 1) {
				console.log('🚫 Single node detected, BLOCKING all RAG context');
				return '';
			}
		}
	}

	// Additional check: if query mentions only VPC, block context
	if (query.toLowerCase().includes('vpc') && !query.toLowerCase().includes('subnet') && !query.toLowerCase().includes('nat')) {
		console.log('🚫 VPC-only query detected, BLOCKING context to prevent templates');
		return '';
	}

	const hasWithScore = typeof vectorStore.similaritySearchWithScore === 'function';
	const k = 1; // return top-1 chunk only

	if (hasWithScore) {
		const results = await vectorStore.similaritySearchWithScore(query, k);
		if (!results || results.length === 0) return null;

		// Filter by similarity score (must be > 0.85)
		const highQualityResults = results.filter(([doc, score]) => score < 0.15); // score is distance, so lower is better
		
		if (highQualityResults.length === 0) {
			console.log('⚠️ Low similarity score, returning empty context');
			return '';
		}
		...
		// For simple similarity search, we can't access scores, so return empty to be safe
		console.log('⚠️ No score available, returning empty context for safety');
		return '';
	}
};
